...
/**
 While throttling the Talon in the positive direction, make sure the sensor
 speed is also positive.
 Additionally note the approximate sensor speed while the Talon is driven.
 getSpeed() will return the speed in RPM since the requirements in Section
 17.2.1 are met.

 The SmartDashboard values can also be read in the roboRIO web-based
 ...
 Remember “err” is in native units per 100ms. So an error of 900 units per 100ms
 equals an error of 131RPM since each rotation is 4096 units.

 Next we will add in P-gain so that the closed-loop can react to error.
 Suppose given our worst error so far (900 native units per 100ms), we want to
 respond with another 10% of throttle.
 Then our starting p-gain would be....
 (10% X 1023) / (900) = 0.113333
 Now let’s check our math, if the Talon SRX sees an error of 900 the P-term will
 be
 ...
 Double the P -gain until the system oscillates (too much) or until the system
 responds adequately.
 If the mechanism is moving to swiftly, you can add D-gain to smooth the motion.
 Start with 10x the p-gain.
 If the mechanism is not quite reaching the final target position (and P -gain
 cannot be increased further without hurting overall performance)
